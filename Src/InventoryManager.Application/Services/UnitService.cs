...
namespace InventoryManager.Application.Services
{
    public class UnitService : IUnitService
    {
        ...
        public IEnumerable<Unit> GetAllUnits(string? searchTerm,string? includeProp = null, int page = 1,int pageSize=1, bool pagination = false, bool descending = false)
        {
            if(searchTerm == null)
            {
                return _unitRepository.GetAll(null,includeProp: includeProp,page,pageSize:pageSize,pagination: pagination, u=>u.Name,descending);
            }

            searchTerm = searchTerm.ToLower();

            return _unitRepository.GetAll(u=>u.Name.ToLower().Contains(searchTerm),includeProp: includeProp,page,pageSize:pageSize, pagination: pagination, u => u.Name, descending);
        }
